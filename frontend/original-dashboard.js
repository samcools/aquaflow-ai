...
(() => {
  ...
  function preferredVoice(lang){
    const list=candidateVoices(lang);
  ...
  function installVoiceStatus(){
    ...
    if(!controls)return;
    if(!controls.querySelector('.voice-picker')){
      controls.insertAdjacentHTML('beforeend',`<div class="voice-picker"><label for="voiceSelect">Ayanda voice</label><select id="voiceSelect" aria-label="Choose Ayanda voice"><option value="auto">Best natural voice</option></select><button id="previewVoiceButton" class="voice-preview-button" type="button" aria-label="Preview selected voice">▶ Preview</button></div>`);
    }
  ...
  function populateVoiceSelect(){
    ...
    voices.forEach((voice,index)=>{
      ...
      const label=voiceQuality(voice,lang);
      option.textContent=`${index===0?'★ ':''}${voice.name} · ${voice.lang} · ${label}`;
      select.appendChild(option);
    });
    select.value=[...select.options].some(o=>o.value===saved)?saved:'auto';
    if(!voices.length){
      const option=document.createElement('option');option.disabled=true;option.value='none';option.textContent='No matching installed voice — system fallback';select.appendChild(option);
    }
  }

  function updateLocaleStatus(){
    ctl.language=selectedLang();
    const voice=preferredVoice(ctl.language),el=byId('voiceLocaleStatus');
  ...
  function selectVoice(){
    const lang=selectedLang(),value=byId('voiceSelect')?.value||'auto';
    localStorage.setItem(voiceKey(lang),value);updateLocaleStatus();
  }

  function previewVoice(){
    const lang=selectedLang();unifiedSpeak(previewText[lang]||previewText['en-ZA'],{force:true});
  }
  ...
  function unifiedSpeak(text,options={}){
    ...
    utterance.lang=lang;if(voice)utterance.voice=voice;
    // Slightly slower than the old voice path for a more natural, conversational cadence.
  ...
  function changeLanguage(){
    ctl.language=selectedLang();populateVoiceSelect();updateLocaleStatus();ctl.commandArmed=false;
  ...
  function wireSingleVoiceControls(){
    ...
    if(byId('voiceSelect'))byId('voiceSelect').onchange=selectVoice;
    if(byId('previewVoiceButton'))byId('previewVoiceButton').onclick=previewVoice;
    if('speechSynthesis'in window)window.speechSynthesis.onvoiceschanged=()=>{populateVoiceSelect();updateLocaleStatus();};
